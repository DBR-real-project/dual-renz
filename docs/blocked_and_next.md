...
**왜 필요한가 (이제 수치가 있다):** 키워드 규칙 기준선을 21건 검증셋(경계 케이스
7건 포함, `docs/validation_report.md` 5장)으로 실측했다.

| 백엔드 | 정탐률 | 오탐률 | 정확도 |
|---|---|---|---|
| 키워드 규칙 (현재 대역) | **63.6%** (7/11) | **30.0%** (3/10) | 66.7% |
| LLM | 미측정 — 키 필요 | | |

규칙 기반은 사기 11건 중 4건을 놓쳤고, **정상 통화 3건을 사기로 오판했다**
(오탐률 30%). 셋 다 같은 이유다 — 부정문을 못 읽는다. 예를 들어 `normal_05`는
"전화로는 계좌번호나 비밀번호를 절대 요구하지 않습니다"라고 **경고하는 정상 은행
상담**인데, 규칙은 "비밀번호"라는 단어만 보고 `개인정보·OTP 요구`에 80점을 준다.
