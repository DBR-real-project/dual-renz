...
모델 크기 선택:
  기본값은 `small`. 실측(강동연, `docs/stt_benchmark.md`): tiny는 clean CER 67.7%로
  base(5.7%)·small(4.5%)과 격차가 크고, 노이즈(SNR 5dB)에서는 아예 무너진다
  (CER 300%+, 콤마 반복 환각). base와 small은 clean 정확도가 거의 같지만(5.7% vs
  4.5%) base가 3배 빠르고(0.79초 vs 2.45초), 대신 노이즈에서는 small이 덜 나빠진다
  (5dB CER 56.5% vs 34.5%). 지금 기본값(small)은 "속도보다 강건성" 쪽 선택인데,
  실시간 스트리밍(Phase 2)을 붙일 때는 base로 내리는 것도 재검토할 만하다.
  `medium` 이상은 CPU에서 너무 느리다(측정 안 함, 체감상 배제).
