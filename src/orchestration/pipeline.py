...
def analyze(
    ...
    if audio_ok:
        # 음성 엔진도 학습 도메인(ASVspoof) 밖에서 무너진다는 걸 실측으로 확인했다.
        # 레벨을 통일해 비교하면 도메인 안은 0점대인데, 실제 통화 녹취와 공개 코퍼스
        # 낭독은 100까지 올라간다(docs/validation_report.md 0-7).
        # 영상과 같은 이유로, 이 점수를 단독 근거로 쓰지 않도록 항상 알린다.
        warnings.append(
            "음성 합성 판별 점수는 ASVspoof 벤치마크에서 검증된 값입니다. "
            "실제 통화 녹취처럼 학습 도메인 밖 음성에서는 진짜 목소리도 합성으로 "
            "판정될 수 있으므로(실측 확인), 음성 점수만으로 판단하지 마세요."
        )
