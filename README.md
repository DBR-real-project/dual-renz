...
| 음성 스푸핑 (AASIST) | ⚠️ | ASVspoof 120건에서 정탐 **97.1%** / 오탐 **0%** / 정확도 **98.3%**. **그러나 실제 통화 녹취 5건을 전부 '합성'으로 판정했다** — 아래 「알려진 한계」 |
...
| AASIST 음성 스푸핑 판별 | ✅ | 정확도 98.3% (120건). 전화망 8kHz 조건도 확인 |
...
| 크롬 확장 실시간 캡처 (MV3 3계층) | ✅ | 실제 크롬에서 캡처→분석→오버레이까지 자동 검증 통과 |
...
| 확장 아이콘 클릭 시 `activeTab` 부여 | 크롬이 **사람의 실제 클릭**에만 준다. 자동화로는 합성 불가 — `verify_extension_chrome.js --manual`로 10초면 확인된다 |
...
- 음성 엔진(AASIST)은 이런 문제가 없다 — 전화망 8kHz 조건에서도 98% 유지

### ★ 실제 보이스피싱 통화에서는 잡는다 — 다만 음성 엔진이 오작동한다

금융감독원이 공개한 **실제 사기범 통화 녹취 5건**을 그대로 넣어봤다.
**5/5 전부 '높음'으로 경고했다** (콘텐츠 위험도가 끌어올림). 이게 지금까지 중
가장 강한 근거다 — 우리가 만든 대본이 아니라 진짜 통화다.

그런데 **미디어 위험도가 5건 모두 100**이었다. 사람이 직접 말한 통화인데
AASIST가 전부 합성으로 봤다. 통제 실험으로 추적한 결과:

| 조건 | AASIST |
|---|---|
| 스튜디오 원본 | 16.5 |
| G.711 전화망(300~3400Hz + 8kHz + μ-law) | **6.5** |
| 실제 공개 녹취 | **100.0** |

**전화망도 코덱도 원인이 아니다**(오히려 점수가 내려간다). 편집·마스킹·화자 혼합·
VoIP 중계 등이 의심되지만 **원인을 규명하지 못했다.**

이 5건에서는 결과적으로 해가 없다(어차피 사기 통화다). 하지만 **정상적인 실제
통화 녹음에서도 같은 일이 일어나면 헛경보가 된다.** 정상 통화 녹취를 구하지 못해
그건 검증하지 못했다. 따라서 **"음성 엔진은 도메인이 바뀌어도 안전하다"고 말하면
안 된다.** 자세한 실험은 [validation_report.md](docs/validation_report.md) 0-7절.
...
- **확장의 `activeTab` 부여만 사람 확인이 남았다.** 나머지(설치·권한·캡처·오버레이·
  정리)는 실제 크롬에서 자동 검증했다. `verify_extension_chrome.js --manual` 참고.
