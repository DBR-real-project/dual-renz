...
| 음성 스푸핑 (AASIST) | ✅ | 정탐률 **97.1%**, 오탐률 **0%**, 정확도 **98.3%** (120건). 8kHz 전화망 조건에서도 유지 |
...
| LLM 연동 경로 | ✅ | 스텁 서버로 **클라이언트 코드 실행 검증 완료** (`scripts/test_llm_client.py`) — 요청 형식·스키마·파싱·폴백. 실제 모델 정확도는 키가 있어야 잰다 |
| LLM 실제 호출 수치 | ⚠️ | `ANTHROPIC_API_KEY` 또는 `GEMINI_API_KEY` 필요. **없어도 오프라인 분류기로 정상 동작한다** |
...
- **전화망 8kHz에서 오탐 여유가 준다.** 정확도 98%는 유지되지만 진짜 음성 최고점이
  2.2 → 30.9로 올라 임계값 50까지 20점밖에 안 남는다.
